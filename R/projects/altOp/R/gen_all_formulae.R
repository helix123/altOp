...
generiere_alle_formeln <- function(abhVar, opList, festVar) {
  stop("unnötige Funktion, siehe Beispiel unten in Quellcode")
...
########################### Anwendungsbeispiele #######################
bsp1_opList <- list(c("roa", "roe", "tsr"))
bsp1_abhVar <- c("compvar")
bsp1_festVar <- c("emp", "marketcap")

bsp1_formulae <- generiere_alle_formeln(abhVar = bsp1_abhVar, opList = bsp1_opList, festVar = bsp1_festVar)

#fällt alles komplett weg, kann mit
form  <- formula(compvar ~ emp + marketcap + x)
sub_vars_rhs(form,c("x"),matrix(c("roa","roe","tsr"),3,1))
#erledigt werden


bsp2_opList <- list(c("roa", "roe", "tsr"), c("marketcap", "emp")) # mehrere alt. Operationalisierungen
bsp2_festVar <- c("beta")

bsp2_formulae <- generiere_alle_formeln(abhVar = bsp1_abhVar, opList = bsp2_opList, festVar = bsp2_festVar)
#fällt alles komplett weg, kann mit
form  <- formula(compvar ~ emp + marketcap + x+y)
sub_vars_rhs(form,c("x"),matrix(c("roa","roe","tsr","marketcap","emp"),3,2))
#erledigt werden
